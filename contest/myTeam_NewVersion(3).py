    ...
    if not gameState.isPacman:
      if len(theirGhosts) > 0:
        for ghost in theirGhosts:
          for i in range(-1, 2):
            for j in range(-1, 2):
              avoided.append((ghost.getPosition()[0] + i, ghost.getPosition()[1] + j))
    else:
      if len(theirGhosts) > 0:
        for ghost in theirGhosts:
          avoided.append(ghost.getPosition())
...
#        print capsules
        ...
        if not self.isSuper:
            ...
            #
            #capsules = [(99,99)]
            print "asddddddddddddd",capsules
                    ...
                    if self.getMazeDistance(capsule, selfState.getPosition()) == dis2closetcap:
                        ...
                        if not len(enemiesAtHome) > 0:#没看到有战斗力的敌人
                            ...
                            if distance < min([self.getMazeDistance(ele.getPosition(), capsule) for ele in enemiesAtHome]) and len(enemies) == 2:
                                # print "decision"
                                if min([self.getMazeDistance(ele.getPosition(), selfState.getPosition()) for ele in enemiesAtHome]) <= 5:
                                    return self.aStarSearch([capsule])
                                else:
