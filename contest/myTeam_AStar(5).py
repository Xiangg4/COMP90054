...
#我又把进攻策略修改了，这回比较激进但是也不容易死，能打赢test3
#################
# Team creation #
#################


#python capture.py -r myTeam_AStar(5) -b myTeam_Test3 -l RANDOM8888 左右blocker会堵死我的路，要排除掉在我家的左右blocker
...
#      print(row)
  ...
  def chooseAction(self, gameState):#只有可能陷入很深的迷宫F:\90054\contest>python capture.py -r myTeam_AStar(5) -b myTeam_Test3 -l RANDOM9999
...
#        print capsules
        ...
        #这次吃药的条件很苛刻，除了敌人在视野外我靠近药物外（安全），当我看到敌人时，只有在我距离药更近而且
        #两个敌人信息都知道的情况下，我考虑才接近药，敌人在视野内，我会全力冲刺药，不在视野内，我会边吃食物边找药
        if not self.isSuper:
                ...
                for capsule in capsules:
                    if self.getMazeDistance(capsule, selfState.getPosition()) == dis2closetcap:
                        ...
                        enemiesAtHome = [ele for ele in enemies if
                                         not ele.isPacman and ele.getPosition() != None and ele.scaredTimer <= 3]  # 我又改成5了
                        ...
                        if not len(enemiesAtHome) > 0:#没看到有战斗力的敌人
                            #return self.aStarSearch([capsule])
                            return self.aStarSearch([self.gameTheoryCalculation1(foodpos, capsule)])#边吃食边找药
                            #python capture.py -r myTeam_NewVersion(3) -b baselineTeam -l RANDOM16 不改会不吃路上东西直奔药，反而什么都吃不到

                        else:#看到有战力的敌人在

                            if distance < min([self.getMazeDistance(ele.getPosition(), capsule) for ele in enemiesAtHome]) and len(enemies) == 2:
                                # print "decision"
                                if min([self.getMazeDistance(ele.getPosition(), selfState.getPosition()) for ele in enemiesAtHome]) <= 7:
                                    return self.aStarSearch([capsule])
                                else:
